...
def gen_bursts(mu_params, sigma_params, mu_dist='uniform', sigma_dist='uniform', 
               N=10000, alpha=-1.5, E_min_log=37, E_max_log=39, name='', save=True):
    
    if mu_dist == 'uniform':
        low = mu_params[0]
        high = mu_params[1]
        mu_f = np.random.uniform(low=low, 
                                 high=high, 
                                 size=N) # MHz
    ...
    if sigma_dist == 'uniform':
        sig_f = np.random.uniform(low=sigma_params[0], 
                                  high = sigma_params[1], 
                                  size=N) # MHz
...
def run_search(bursts, fstart, fend, fluence_threshold, 
               in_band_sig=1, ret='all', distance=972):
    """
    fluence_threshold: in Jy ms 
    fstart, fend: MHz
    bursts: dataframe
    distance: Mpc
    """
    ...
    # detected_snr_E = np.array(detected_df['snr_S'] * 2.355 * detected_df['in_sig_f'])
    ...
    detected_snr_E = np.array(fluence_to_energy(detected_df['snr_S'], freq_width, distance))
...
def analyse_and_plot(bursts, detected, Es):
    ...
    print(f'Recovered sig_f is: {qs[1]:.2f}+{(qs[2] - qs[1]):.2f}-{(qs[1] - qs[0]):.2f}')
    ...
    axes[0].hist(detected['in_mu_f'], alpha=0.5, density=True, label='detected', bins=bins)
    ...
    v, bins, _ = axes[1].hist(bursts['in_sig_f'], alpha=0.5, density=True, bins=30, label='in')
    axes[1].hist(detected['in_sig_f'], alpha=0.5, density=True, label='detected', bins=bins)
    ...
    axes[2].hist(np.log10(detected_fit_E), bins=30, density=True, alpha=0.5,
                 label='fit_E')
    axes[2].hist(np.log10(detected_in_band_E), bins=30, density=True, alpha=0.5, 
                 label='in_band_E')
    axes[2].set_yscale('log')
