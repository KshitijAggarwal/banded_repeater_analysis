...
def gauss_integral(fstart, fend, mu_f, sig_f):
    """
    Integrate a gaussian function within limits

    fstart: start frequency of the band
    fend: end frequency of the band
    mu_f: mean of the gaussian
    sig_f: sigma of the gaussian
...
def energy_to_fluence(E, sig_f, distance=972):
    """
    Convert energy to fluence using burst bandwidth
    and distance
...
def fluence_to_energy(S, freq_width, distance=972):
    """
    Convert fluence to energy using burst bandwidth
    and distance
...
def fit_double_power_law(Ns, Es):
    popt, perr = curve_fit(
        ...
        Es,
        Ns,
        bounds=([-np.inf, 10 ** (0), -5, -5], [np.inf, 10 ** (4), 5, 5]),
    )
    slope1 = popt[-2]
    slope2 = popt[-1]
    scale = popt[0]
